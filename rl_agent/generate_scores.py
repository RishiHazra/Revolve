...
def fitness_function(
    collision: bool,
    current_speed: float,
    min_distance_to_waypoint: float,
    desired_speed_min: float = 9.0,
    desired_speed_max: float = 10.5,
    collision_penalty: float = -1,
    speed_penalty: float = 0,
    speed_factor: float = 1.0,
    distance_factor: float = 1.0,
    speed_tolerance: float = 2,
    max_distance_tolerance: float = 1,
) -> float:
    ...
    if adjusted_min_speed <= current_speed <= adjusted_max_speed:
        ...
        deviation = min(
            abs(current_speed - adjusted_min_speed),
            abs(current_speed - adjusted_max_speed),
        )
        speed_reward = max(
            speed_factor * (1 - deviation / (adjusted_max_speed - adjusted_min_speed)),
            0,
        )
    ...
    if min_distance_to_waypoint <= max_distance_tolerance:
        ...
        distance_penalty = max(
            0, distance_factor * (1 - excess_distance / max_distance_tolerance)
        )
    ...
    print(
        "min_distance_to_waypoint,distance_reward",
        min_distance_to_waypoint,
        distance_reward,
    )
...
class DrivingAgent:
    def __init__(self, no_model, iteration, group_id, baseline):
        ...
        if loop_num != 0:
            self.epsilon = 0.0
            self.q_net.load_weights(
                f"{os.environ['ROOT_PATH']}/{baseline}_database/gpt-4/group_{group_id}/"
...
def generate_behaviour(reward_func_path, counter_model, iteration, group_id, baseline):
    ...
    agentoo7 = DrivingAgent(
        counter_model, iteration, group_id, baseline
    ...
    while total_step_counter <= max_steps:
        ...
        while not done:
            ...
            if episodic_steps > 5:
                step_fitness = fitness_function(
                    collision=collision_info.has_collided,
                    current_speed=speed,
                    min_distance_to_waypoint=min_pos,
                    speed_tolerance=1.5,
                )
        ...
        print(
            "final episode reward , with epsilon probability and total steps ",
...
rew_path = os.path.join(
    os.environ["ROOT_PATH"],
    f"{baseline}_database/gpt-4/group_{group_id}/reward_fns/{iteration}_{model_number}.txt",
)
average_fitness = generate_behaviour(
    rew_path,
    counter_model=model_number,
    iteration=iteration,
    group_id=group_id,
    baseline=baseline,
)  # def run_training( reward_func_path, counter_model, iteration, group_id):
...
if saving == 1:
    file_path = os.path.join(
        os.environ["ROOT_PATH"],
        f"{baseline}_database/gpt-4/group_{group_id}/"
