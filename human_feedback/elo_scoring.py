...
def group_feedback(df):
    ...
    for _, row in df.iterrows():
        video_1, pos_feedback_1, neg_feedback_1 = row['Video 1'], row['Positive Feedback 1'], row['Negative Feedback 1']
        video_2, pos_feedback_2, neg_feedback_2 = row['Video 2'], row['Positive Feedback 2'], row['Negative Feedback 2']
...
def main():
    ...
    parser.add_argument('--gen_id', required=True, type=int, help='Run the application in dummy mode')
    parser.add_argument('--baseline', required=True, type=str,
                        choices=['hf_database', 'hf_eureka_database'], help='name of database?')
    args = parser.parse_args()
    GEN_ID = args.gen_id
    baseline = args.baseline
    response_filename = f'responses_*.csv'
    load_dir = os.path.join(os.environ['ROOT_PATH'], 'human_feedback', f'generation_{GEN_ID}')
    response_paths = glob.glob(f"{load_dir}/{response_filename}")
    # also load responses from prev gens
    if GEN_ID > 0:
        for id in range(GEN_ID):
            load_dir = os.path.join(os.environ['ROOT_PATH'], 'human_feedback', f'generation_{id}')
            response_paths += glob.glob(f"{load_dir}/{response_filename}")
    df = pd.DataFrame(columns=['Video 1', 'Video 2', 'Selected',
    ...
    for _, row in combined_df.iterrows():
        path, hf_fitness_score, pos_feedback, neg_feedback = (row['Video'], row['Score'],
                                                              ', '.join(row['Positive Feedback']),
                                                              ', '.join(row['Negative Feedback']))
        reg_str = '\\' if '\\' in path else '/'
        group_id, it, running_number = [int(x) for x in
                                        path.split(reg_str)[-1].replace('.mp4', '').replace('gr', '').split('_')]
        # assert it == GEN_ID, "bug in saving hf fitness score"
        database_dir = os.path.join(os.environ['ROOT_PATH'], f"hf_database/{model_name}/group_{group_id}")
        filename_suffix = f"{it}_{running_number}"
        reward_fn_filename = os.path.join(database_dir, f"reward_fns/{filename_suffix}.txt")
        save_fitness_score(hf_fitness_score, model_name, group_id, it, running_number, baseline)
        human_feedback = f'{pos_feedback}\n{neg_feedback}'
        save_human_feedback(human_feedback, model_name, group_id, it, running_number, baseline)
        if it > 0:
            # now load groups
